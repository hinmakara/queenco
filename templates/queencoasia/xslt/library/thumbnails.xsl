...
<xsl:stylesheet	version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:umi="http://www.umi-cms.ru/TR/umi">
	...
	<xsl:template name="banner">
		...
		<xsl:apply-templates select="document(concat('udata://system/makeThumbnailFull/(.', $src, ')/', $width, '/', $height, '/void/0/1/5/', $quality))/udata" mode="banner">
			<xsl:with-param name="quality" select="$quality" />
		</xsl:apply-templates>
	</xsl:template>

	<xsl:template match="udata[@module = 'system' and (@method = 'makeThumbnail' or @method = 'makeThumbnailFull')]" mode="banner">
		<img src="{src}" data-bgfit="cover" data-bgposition="center center" data-bgrepeat="no-repeat" alt="slidebg2"/>
	</xsl:template>
	...
	<xsl:template name="thumbnail">
		...
		<xsl:apply-templates select="document(concat('udata://system/makeThumbnailFull/(.', $src, ')/', $width, '/', $height, '/void/0/1/5/', $quality))/udata">
			<xsl:with-param name="element-id" select="$element-id" />
			<xsl:with-param name="field-name" select="$field-name" />
			<xsl:with-param name="empty" select="$empty" />
			<xsl:with-param name="align" select="$align" />
