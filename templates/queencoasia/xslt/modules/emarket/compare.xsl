...
<xsl:stylesheet	version="1.0"
	...
	<xsl:template match="field[@type = 'price']" mode="compare">
        <tr>
            <td class="name">
                <xsl:value-of select="@title" />
            </td>
            <xsl:apply-templates select="values/item" mode="otherprice" />
        </tr>
    </xsl:template>
	...
	<xsl:template match="item" mode="compare">
		<td>
			<xsl:apply-templates select="value[@id]" />
		</td>
	</xsl:template>

    <xsl:template match="item" mode="otherprice"><td></td></xsl:template>
    <xsl:template match="item[value/value != 0]" mode="otherprice">
		<td>
            <div class="oldprice">
                <span>
                    <xsl:apply-templates select="document(concat('udata://emarket/price/', @id))" mode="old_price">
                        <xsl:with-param name="price" select="value/value" />
                    </xsl:apply-templates>
                </span>
            </div>
		</td>
