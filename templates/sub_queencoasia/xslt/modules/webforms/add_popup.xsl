...
<xsl:stylesheet	version="1.0"
    ...
    <xsl:template match="udata[@module = 'webforms'][@method = 'add']">
        <form method="post" action="/webforms/send/" onsubmit="site.forms.data.save(this); return site.forms.data.check(this);" enctype="multipart/form-data">
            <label>Оставьте вашу заявку и наш оператор свяжется с вами в ближайшее время.</label>
    ...
    <xsl:template match="field[@type = 'file' or @type = 'img_file' or @type = 'swf_file' or @type = 'video_file']" mode="webforms_input_type">
        <span class='i18n'>max-file-size</span><xsl:value-of select="@maxsize" />Mb
